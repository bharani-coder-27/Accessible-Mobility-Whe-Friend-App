  ...
  return (
    <View>
      ...
      <View style={{ alignItems: 'center', padding: 25 }}>
        <Text style={{ fontSize: 25 }}>{status.charAt(0).toUpperCase() + status.slice(1)} Passengers</Text>
      </View>
      <FlatList
        data={filtered}
        ...
        renderItem={({ item }) => (
          ...
          <View>
            ...
            <Text style={styles.details}>Stop: {item.bus_stop_name} | Time: {item.timing}</Text>
          </View>
          {status === 'waiting' && (
      ...
      ListEmptyComponent={() => (
        <View style={styles.empty}><Text>No passengers</Text></View>
      )}
    />
    </View>
  );
...
const styles = StyleSheet.create({
  item: { padding: 15, flexDirection: 'row', justifyContent: 'space-between', borderBottomWidth: 1, borderColor: '#eee' },
  name: { fontSize: 16, fontWeight: '500' },
  details: { fontSize: 14, color: '#555' },
  button: { backgroundColor: '#624be4ff', padding: 8, borderRadius: 6, alignContent: 'center', justifyContent: 'center' },
  buttonText: { color: '#fff' },
  empty: { marginTop: 50, alignItems: 'center' },
  countContainer: { width: 130, height: 45, borderRadius: 8, padding: 10, backgroundColor: '#ffffffff', alignItems: 'center', borderWidth: 1 },
